...
contract RentalContract {
    struct User {
        ...
        string email; // Kullanıcı e-posta
    }
    ...
    event UserRegistered(address indexed userAddress, string userType, string name, string email);
    ...
    // Kullanıcı kaydı
    function registerUser(string memory _userType, string memory _name, string memory _email) public {
        require(bytes(users[msg.sender].userType).length == 0, "User already registered");
        users[msg.sender] = User(_userType, _name, _email);
        emit UserRegistered(msg.sender, _userType, _name, _email);
    }
